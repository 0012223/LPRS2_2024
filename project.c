...
#define NUMBER_OF_CHARACTERS 85

///////////////////////////////////////////////////////////////////////////////
// HW stuff.

#define WAIT_UNITL_0(x) \
	while (x != 0)      \
	{                   \
	}
#define WAIT_UNITL_1(x) \
	while (x != 1)      \
	{                   \
	}
...
const Vector2 letter_postions[] = {
	{0, 0}, {6, 0}, {12, 0}, {18, 0}, {23, 0}, {31, 0}, {38, 0}, {43, 0}, {52, 0}, {58, 0}, {64, 0}, {69, 0}, {75, 0}, {82, 0}, {92, 0}, {99, 0}, {105, 0}, {114, 0}, {120, 0}, {126, 0}, {132, 0}, {138, 0}, {144, 0}, {151, 0}, {0, 9}, {8, 9}, {14, 9}, {21, 9}, {27, 9}, {34, 9},
...
void render_letter(uint8_t letter_index, Vector2 pos)
{
	...
	for (uint16_t i = 0; i < 9; ++i)
	{
		for (uint16_t j = 0; j < letter_width; ++j)
		{
			...
			uint8_t overlay_r = 0b111;
			uint8_t overlay_g = 0b111;
			uint8_t overlay_b = 0b111;
...
int main(void)
{
	...
	Vector2 text_pos = {10, 80};
	Vector2 text_velocity = {0, 0};

	uint16_t angle = 0;
	uint16_t animation_tick = 0;

	uint8_t loading_percentage = 0;
	uint8_t loaded = 0;

	char text[] = "Немања Милошевић РА/200\nАлекса Станковић РА/219\nМарко Пушковић РА/230"; // „ “
	...
	while (1)
	{
		...
		if (loading_percentage > 108)
		{
			loaded = 1;
			loading_percentage = 108;
		}
		...
		if (loaded == 0)
		{
			for (uint16_t i = 0; i < 50; ++i)
			{
				for (uint16_t j = 0; j < 50; ++j)
				{
					...
					unpack_rgb333_p32[(SCREEN_RGB333_H / 2 - 45 + i) * SCREEN_RGB333_W + SCREEN_RGB333_W / 2 - 25 + j] = rgb333;
				}
			}
			...
			Vector2 new_start = {26, 71};
			Vector2 new_end = {26 + loading_percentage, 74};

			fill_rect_rgb333(start, end, (Color){0xff, 0xff, 0xff});
			fill_rect_rgb333(new_start, new_end, (Color){0x00, 0x00, 0x00});
		}
		else
		{
